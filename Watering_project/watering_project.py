...
def get_df():
    ...
    try: #if amount data/ day is exceededd the file doesn't follow expected pattern
        df=pd.read_csv('weather.csv',header=0,  usecols=[1,5,6], nrows=24)
    
    except:
        print ('loading backup weather file')
        
        df=pd.read_csv('weather_backup.csv',header=0,  usecols=[1,5,6], nrows=24)
...
def str_to_log(h):
    ...
    with open ('log_watering.csv' , 'a') as ff:
...
delta=timedelta(hours=12)  #just to check if while loop works, in the future while True
nu=datetime.now()
while nu+delta>datetime.now():
    ...
    if now.minute == 0 and now.second == 0 and now.hour !=7: 
